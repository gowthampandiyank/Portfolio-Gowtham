...
SELECT age_group, COUNT(*) FROM employess_basics GROUP BY age_group;
...
FROM employee_managers m
    JOIN (
        SELECT DISTINCT
            manager_id, deportment
        FROM employees_working_details
    ) e ON m.manager_id = e.manager_id
GROUP BY
    m.manager_id,
    m.manager_name
...
FROM (
    ...
    LEFT JOIN dbo.employees_working_details e ON d.deportment = e.deportment
WHERE
...
WHERE
    TABLE_NAME = 'employess_basics';

-- <5> Age & Demographics

-- 5.1 Find the average age by employee type (Intern, Full-Time, etc.).
SELECT w.employee_type, AVG(b.age) AS avg_age
FROM dbo.employess_basics b
    JOIN dbo.employees_working_details w ON b.employee_id = w.employee_id
GROUP BY
    w.employee_type;

-- 5.2 Count employees who are above 50 years old in each department.
SELECT w.deportment, COUNT(*) AS employees_above_50
FROM dbo.employess_basics b
    JOIN dbo.employees_working_details w ON b.employee_id = w.employee_id
WHERE
    b.age > 50
...
SELECT w.deportment, SUM(
        CASE
            WHEN b.gender = 'Male' THEN 1
            ELSE 0
        END
    ) AS male_count, SUM(
        CASE
            WHEN b.gender = 'Female' THEN 1
            ELSE 0
        END
    ) AS female_count
FROM dbo.employess_basics b
...
SELECT w.manager_id, MIN(b.age) AS youngest_age, MAX(b.age) AS oldest_age
FROM dbo.employess_basics b
    JOIN dbo.employees_working_details w ON b.employee_id = w.employee_id
GROUP BY
    w.manager_id;
...
SELECT deportment, AVG(working_hours) AS avg_working_hours
FROM dbo.employees_working_details
GROUP BY
    deportment
